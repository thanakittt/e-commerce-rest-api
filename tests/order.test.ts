...
const insertTestOrder = async (
  ...
  const [order] = await sql<[TestOrder]>`
    ...
    RETURNING id, status, shipping_address as "shippingAddress", payment_method as "paymentMethod", order_date as "orderDate"
  `;
  return order;
};
...
describe("GET /api/orders", () => {
  describe("Success Cases (200 OK)", () => {
    ...
    it("should return 200 with all orders and aggregated totals for authenticated user", async () => {
      ...
      const productB = await insertTestProduct({ name: "Product B", price: 50 });

      // Order 1 items
      const orderItem1A = await insertTestOrderItem(order1.id, productA.id, 1, productA.price);
      const orderItem1B = await insertTestOrderItem(order1.id, productB.id, 2, productB.price);

      // Order 2 items
      const orderItem2A = await insertTestOrderItem(order2.id, productA.id, 3, productA.price);
      const orderItem2B = await insertTestOrderItem(order2.id, productB.id, 1, productB.price);

      const subtotal1A = orderItem1A.quantity * orderItem1A.unitPrice;
      const subtotal1B = orderItem1B.quantity * orderItem1B.unitPrice;
      const totalPrice1 = subtotal1A + subtotal1B;
      const totalQuantity1 = orderItem1A.quantity + orderItem1B.quantity;

      const subtotal2A = orderItem2A.quantity * orderItem2A.unitPrice;
      const subtotal2B = orderItem2B.quantity * orderItem2B.unitPrice;
    ...
    it("should return orders sorted by orderDate DESC and items sorted by productId ASC", async () => {
      ...
      const productLowId = await insertTestProduct({ name: "Low ID Product", price: 10 });
      const productHighId = await insertTestProduct({ name: "High ID Product", price: 20 });

      // Insert items with high ID first to verify ASC order
      await insertTestOrderItem(olderOrder.id, productHighId.id, 1, productHighId.price);
      await insertTestOrderItem(olderOrder.id, productLowId.id, 1, productLowId.price);

      await insertTestOrderItem(newerOrder.id, productHighId.id, 2, productHighId.price);
      await insertTestOrderItem(newerOrder.id, productLowId.id, 1, productLowId.price);
    ...
    it("should correctly calculate totalPrice and subtotal with decimal unit prices", async () => {
      ...
      const productA = await insertTestProduct({ name: "Product A", price: 19.99 });
      const productB = await insertTestProduct({ name: "Product B", price: 9.95 });
    ...
    it("should correctly handle various order statuses and payment methods", async () => {
      ...
      await insertTestOrderItem(cancelledOrder.id, product.id, 1, product.price);
